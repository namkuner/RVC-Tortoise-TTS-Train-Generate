...
class TrainingState():
	...
	def parse_metrics(self, data):
		if isinstance(data, str):
			if data.find('Training Metrics:') >= 0:
				data = json.loads(data.split("Training Metrics:")[-1])
				data['mode'] = "training"
			elif data.find('Validation Metrics:') >= 0:
				data = json.loads(data.split("Validation Metrics:")[-1])
